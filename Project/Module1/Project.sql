...
CREATE TABLE AssetAssignedHistory (
	...
	SELECT * FROM HardwareAssets
	WHERE PurchasePrice > 500;

	SELECT * FROM HardwareAssets
	WHERE PurchasePrice > 500 AND AssetStatus = 1;

	SELECT A.AssetName, B.AssetType
	FROM HardwareAssets A
	INNER JOIN MasterAssetType B ON A.AssetType = B.AssetTypeID;

	SELECT A.AssetType,
		   MAX(B.PurchasePrice) AS MaxPrice,
		   MIN(B.PurchasePrice) AS MinPrice
	FROM HardwareAssets B
	INNER JOIN MasterAssetType A ON B.AssetType = A.AssetTypeID
	GROUP BY A.AssetType;

	SELECT B.AssetStatus, COUNT(A.SoftwareAssetID) AS AssetCount
	FROM SoftwareAssets A
	INNER JOIN MasterAssetStatus B ON A.AssetStatus = B.AssetStatusID
	GROUP BY B.AssetStatus
	HAVING COUNT(A.SoftwareAssetID) > 1;

	SELECT AssetName, PurchasePrice
	FROM SoftwareAssets
	...
	SELECT AssetName, 
		   PurchasePrice,
		   CASE 
			   WHEN AssetStatus = 1 THEN 'Assigned'
			   WHEN AssetStatus = 2 THEN 'UnAssigned'
			   WHEN AssetStatus = 3 THEN 'Stock'
			   WHEN AssetStatus = 4 THEN 'Faulty/Repair'
			   WHEN AssetStatus = 5 THEN 'End of Life'
			   ELSE 'Unknown Status'
