...
public class HardwareAssignedService {
	 ...
	 private static HardwareAssignedImpl hardwareAssignedImpl;
	    private static Scanner scanner = new Scanner(System.in);
	    
	    public HardwareAssignedService(HardwareAssignedRepository HardwareAssignedRepo) {
	        this.hardwareAssignedImpl = new HardwareAssignedImpl(HardwareAssignedRepo);
	    }
        ...
        hardwareAssignedImpl.addHardwareAssigned(new HardwareAssigned(id, assetID, employeeID, assignedBy, date, status));
        ...
        int id = getValidInt();
        hardwareAssignedImpl.removeHardwareAssigned(id);
        ...
        String status = scanner.next();
        hardwareAssignedImpl.updateHardwareAssigned(new HardwareAssigned(id, assetID, employeeID, assignedBy, date, status));
        ...
        int id = getValidInt();
        HardwareAssigned assigned = hardwareAssignedImpl.getHardwareAssigned(id);
        if (assigned != null) {
        ...
        List<HardwareAssigned> assignedList = hardwareAssignedImpl.getAllHardwareAssigned();
        // Print table header
        System.out.printf("%-10s %-15s %-15s %-20s %-20s %-15s%n", 
                          "ID", "Asset ID", "Employee ID", "Assigned By", "Assigned Date", "Status");
        System.out.println("----------------------------------------------------------------------------------------");
